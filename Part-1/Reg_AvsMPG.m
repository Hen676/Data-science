...
%% Ridge
lambda = 1e-2;
Ridge = ridge(DataTrain.mpg,DataTrain.acceleration,lambda,0);
%% Lasso
[LassoALL,Lstats] = lasso(DataTrain.acceleration,DataTrain.mpg,'CV',20);
LassoInt = Lstats.Intercept(Lstats.Index1SE);
LassoGrad = LassoALL(:,Lstats.Index1SE);
%% Y predict values
yLinear = (DataTrain.acceleration * gradient) + intercept;
yRidge = (DataTrain.acceleration * Ridge(2)) + Ridge(1);
yLasso = (DataTrain.acceleration * LassoGrad) + LassoInt;
%% Y predict values for test
PreyLinear = (DataTest.acceleration * gradient) + intercept;
PreyRidge = (DataTest.acceleration * Ridge(2)) + Ridge(1);
PreyLasso = (DataTest.acceleration * LassoGrad) + LassoInt;
%% least-square errors
